...
A function may be defined by different formulas on different portions of the $x$-axis. Such a function is said to be defined *piecewise*. In other words, we are defining the function piece by piece.

To graph a function defined piecewise, we consider each piece of the $x$-axis separately.
#### $\ex$ – Evaluating Piecewise Function
Suppose we're given the function $$f(x)=\begin{cases}
x+0 & \text{if}\,-2\leq x\leq 3 \\
4-x & \text{if}\,3< x\leq 8
\end{cases}$$Evaluate $f(-1)$ and $f(5)$.

##### Solution.
(a) We have that the input value here is $x=-1$, which satisfies $-2\leq x\leq3$. Therefore, we will use the first equation in the piecewise function to evaluate: $$f(-1)=-1+0=-1.$$
(b) We have that the input value here is $x=5$, which satisfies $3< x\leq8$. Therefore, we will use the second equation in the piecewise function to evaluate: $$f(5)=4-5=-1.$$
#### $\ex$ – Graphing Piecewise Function
Graph the function defined by $$f(x)=\begin{cases}
x+1& \text{if}\, x\leq 1 \\
3& \text{if}\, x< 1
\end{cases}$$
##### Solution.
Think of the plane as divided into two regions by the vertical line $x=1$, as seen in the figure below. In the left-hand region $(x\leq1)$, we graph the line $y=x+1$. (The fastest way to graph the line is to plot its intercepts, $(-1,0)$ and $(0,1)$.)
...
In the right-hand region ($x>1$), we graph the horizontal line $y=3$. The value $x=1$ is not included in the second region, so the point $(1,3)$ is not part of the graph. We indicate this with an open circle at the point $(1,3)$.
...
Then the domain of the piecewise function is the concatenation (stick them together with a union $\cup$) of these domains, and the range of the piecewise function is the concatenation of the ranges of the functions on their corresponding domains.
#### $\ex$ – Domain and Range of Piecewise Function
See [Example 145](https://mathbooks.unl.edu/PreCalculus/piecewisefunctions.html)
## Intro to Composite and Inverse Functions
#### $\defn$ – Composite Function
Let $f(x)$ and $g(x)$ be functions. A *composite* function is of the form $g(f(x))$.
#### $\ex$ – Evaluating Composite Functions
Given the functions $f(x)=x^2-2x$ and $g(x)=0.5x$, identify the function $f(g(x))$ and find the value of $f(g(1))$

##### Solution.
First, let's find the function $f(g(x))$. To identify this function, we are plugging in $0.5x$ for $x$ in the function $f(x)=x^{2}-2x$. We get the function $$f(g(x))=f(0.5x)=(0.5x)^{2}-2(0.5x)=0.25x^{2}-x$$Now, to find the value $f(g(1))$ we evaluate the function $f(g(x))$ at $x=1$. We have $$f(g(1))=0.25(1)^{2}-1=-0.75.$$
#### $\ex$ 
![[friends.jpg]]
#### $\defn$ – Inverse Function
Suppose the inverse of $f$ is a function, denoted by $f\inv$. Then $$f\inv(y)=x \,\text{if and only if}\, f(x)=y.$$ Given a function $f(x)=y$, we find its inverse, should one exist, by solving for $x$ in terms of $y$. 
#### $\war$ 
Although the same symbol, $\inv$, is used for both reciprocals (negative exponent) and inverse functions, the two notions are _not_ equivalent. That is, the inverse of a given function is usually not the same as the reciprocal of that function.
#### $\ex$ – Finding Inverse Function
If $y=f(x)=x^{3}+2$, find $f\inv(10)$.

##### Solution. 
We first find the inverse function for $y=x^{3}+2$ by solving for $x$: $$\begin{align*}
y&= x^{3}+2\\
y-2&= x^{3}\\
\sqrt[3]{y-2}&= x
\end{align*}$$The inverse function is $x=f\inv(y)=\sqrt[3]{y-2}$. Now we evaluate the inverse function at $y=10$: $$f\inv(10)=\sqrt[3]{(10)-2}=\sqrt[3]{8}=2.$$
#### $\rem$ – Domain and Range of Inverse Functions
When we interchange the input and output variables to obtain the inverse function, we interchange the domain and range of the function.
#### $\rem$ 
Suppose $f\inv$ is the inverse function for $f$. Then $$f\inv (f(x))=x \quad\text{and}\quad f(f\inv(y))=y$$as long as $x$ is in the domain of $f$, and $y$ is in the domain of $f\inv$.
# Exponential Functions
## Exponential Functions
#### $\defn$ – Exponential Function
An exponential function is a function of the form $$P(t)=a(b)^{t},\,\text{where}\, b>0 \,\text{and}\, b\neq1, a\neq0.$$The constant $a$ is the $y$-value of the $y$-intercept of the function.

#### $\defn$ – Growth Rate / Factor
The positive constant $b$ is called the *base* or the *growth factor* of the exponential function. We can also define the *growth rate* of the function to be the quantity $r=b-1$, which is usually expressed as a percentage value.

## $\thm$ – Properties of Exponential Functions
Let $f(x)=a(b)^{t}$ be an exponential function with $a>0$. Then the following hold:
- The domain of $f(x)$ is $(-\infty,\infty)$.
- The range of $f(x)$ is $(0,\infty)$.
- If $b>1$ (equivalently $r>0$), the function is increasing.
- If $0<b<1$ (equivalently $-1<r<0$), the function is decreasing.

#### $\defn$ – Exponential Growth / Decay
## Let $f(x)=a(b)^{t}$ be an exponential function with $a>0$. 

If $b>1$ (equivalently $r>0$), the function is said to have *exponential growth*.
If $0<b<1$ (equivalently $-1<r<0$), the function is said to have *exponential decay*.

#### $\defn$ – Exponential Equation
An *exponential equation* is one in which the variable is part of an exponent. 

#### $\ex$ – Solving Exponential Equations
Solve $3^{x-2}=9^{3}$.

##### Solution.
Using the fact that $9=3^{2}$, we write each side of the equation as a power of $3$: $$\begin{align*}
3^{x-2}&=9^{3}\\
3^{x-2}&=(3^{2})^{3}\\
3^{x-2}&=3^{6}
\end{align*}$$Now we equate the exponents to obtain $x-2=6$, and thus $x=8$.
